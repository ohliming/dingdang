...
$score = getUrlParam('score',$str_url);
$alias = getUrlParam('alias',$str_url);
$content = getUrlParam('content',$str_url);

$json_arr = array();

$insert_sql = "INSERT INTO comment(good_id,score,alias,content,uid) VALUES($good_id,$score,'$alias','$content','$uid')";
if(!mysql_query($insert_sql,$dbcon)){
    $msg = "add comment failed：".mysql_error();
    $ret = 1;
} else {
