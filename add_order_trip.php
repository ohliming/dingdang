...
$insert_sql = "INSERT INTO goods_book(initiator_id,tour_line,title,content,guid,tuid,type,count,price,address,start_time,end_time,days) 
			VALUES($guid,'$tour_line','$title','$content',$guid,$tuid,1,$count,$price,$address,'$start_time','$end_time',$days)";

if(!mysql_query($insert_sql,$dbcon)){
    $msg = "add order trip failed：".mysql_error();
    $ret = 1;
} else {
    $msg = "add order trip success";
    $ret = 0;
}
